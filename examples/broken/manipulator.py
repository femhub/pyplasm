...
class PyViewer(GLCanvas):
    ...
    def onKeyboard(self, key, x, y):
        if key == ord("t") or key == ord("T"):
            self.manipulator.setOperation(Manipulator.PLASM_TRANSLATE)
            self.manipulator.setObject(self.box, self.T)
            self.Redisplay()
            return

        if key == ord("s") or key == ord("S"):
            self.manipulator.setOperation(Manipulator.PLASM_SCALE)
            self.manipulator.setObject(self.box, self.T)
            self.Redisplay()
            return

        if key == ord("r") or key == ord("R"):
            manipulator.setOperation(Manipulator.PLASM_ROTATE)
            self.manipulator.setObject(self.box, self.T)
            self.Redisplay()
            return
    ...
    def renderOpenGL(self):
        self.clearScreen()
        self.setViewport(
            self.frustum.x, self.frustum.y, self.frustum.width, self.frustum.height
        )
